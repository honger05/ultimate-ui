<template>
  <cell v-show="showCell" :title="title" primary="content" is-link :inline-desc="inlineDesc" @click="onClick">
    <span class="ci-popup-picker-value" v-if="!showName && value.length">{{value | array2string}}</span>
    <span class="ci-popup-picker-value" v-else="showName && value.length">{{value | value2name data}}</span>
    <span v-if="!value.length && placeholder" v-html="placeholder"></span>
  </cell>
  <popup :show.sync="show" class="ci-popup-picker" :id="'ci-popup-picker-'+uuid" @on-hide="onPopupHide" @on-show="$emit('on-show')">
    <div class="ci-popup-picker-container">
      <div class="ci-popup-picker-header">
        <flexbox>
      ...
      :container="'#ci-popup-picker-'+uuid"></picker>
    </div>
  </popup>
</template>
...
import FlexboxItem from './flex/item'
...
.ci-popup-picker-header {
  height: 44px;
  color: #0ae;
}
.ci-popup-picker-value {
